...
import { actionSaveCloudTemplate } from "@/lib/forms/template-actions";
import {
  TEMPLATE_CATEGORIES,
...
import type { FormTemplateCategory } from "@/lib/db/schema";
...
type Props = {
  ...
  /** When set, "This Website" destination is enabled. */
...
/**
 * Save as Template — destination picker (ADR-007 Step 1).
 */
export function SaveTemplateDialog({
  ...
  const [name, setName] = useState(snapshot.name || "My template");
  const [description, setDescription] = useState("");
  const [destination, setDestination] =
    useState<TemplateSaveDestination>("organization");
  const [category, setCategory] = useState<FormTemplateCategory>("lead");
  ...
  function submit() {
    ...
    startTransition(async () => {
      const result = await actionSaveCloudTemplate({
        destination,
        name,
        description,
        ...
        category,
        tagsRaw,
        sourceFormId: snapshot.sourceFormId,
        clientId: clientId ?? snapshot.clientId,
        websiteId: websiteId ?? snapshot.websiteId,
      });
      if (!result.ok) {
        ...
        <div className="flex-1 space-y-3.5 overflow-y-auto px-4 py-4">
          ...
          <div>
            ...
            <div className="flex flex-col gap-1.5">
              {TEMPLATE_SAVE_DESTINATIONS.map((d) => {
                  ...
                  >
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="block text-[11.5px] text-faint">
                        {needsSite
                          ? "Open this form from a website to use this option."
                          : !allowed
          ...
          <div className="grid grid-cols-2 gap-2">
            <label className="block">
              ...
              <select
                value={category}
                onChange={(e) =>
                  setCategory(e.target.value as FormTemplateCategory)
                }
