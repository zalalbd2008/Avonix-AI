...
import { mergeUptimeSettings, type UptimeSettings } from "./types";
...
export async function actionSaveUptime(input: {
  websiteId: string;
  clientId: string;
  ...
  const uptime = mergeUptimeSettings(input.settings);

  if (uptime.expectedStatusCodes.length === 0) {
    return {
      ok: false,
      error: "Add at least one expected HTTP status code (e.g. 200).",
    };
  }
